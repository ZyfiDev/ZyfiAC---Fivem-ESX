...
local lastduree          = ""
local lasttarget         = ""
local zyfiAC_banlist            = {}
local zyfiAC_banlistLoad        = false
local zyfiAC_banlistHistory     = {}
local zyfiAC_banlistHistoryLoad = false
...
TriggerEvent('es:addGroupCommand', 'banreload', Config.permission, function (source)
  zyfiAC_banlistLoad        = false
  zyfiAC_banlistHistoryLoad = false
  Wait(5000)
  if zyfiAC_banlistLoad == true then
	TriggerEvent('bansql:sendMessage', source, _U('zyfiAC_banlist_loaded'))
	if zyfiAC_banlistHistoryLoad == true then
		TriggerEvent('bansql:sendMessage', source, _U('banhistory_loaded'))
	end
  else
	TriggerEvent('bansql:sendMessage', source, _U('zyfiAC_banlist_starterror'))
  end
end)

TriggerEvent('es:addGroupCommand', 'banhistory', Config.permission, function (source, args, user)
 if args[1] ~= nil and zyfiAC_banlistHistory ~= {} then
	local nombre = (tonumber(args[1]))
	local name   = table.concat(args, " ",1)
	if name ~= "" then

			if nombre ~= nil and nombre > 0 and zyfiAC_banlistHistory[nombre] ~= nil then
					local expiration = zyfiAC_banlistHistory[nombre].expiration
					local timeat     = zyfiAC_banlistHistory[nombre].timeat
					local calcul1    = expiration - timeat
					local calcul2    = calcul1 / 86400
					local calcul2 	 =  math.ceil(calcul2)
					local resultat   = (tostring(zyfiAC_banlistHistory[nombre].targetplayername)) .. " , " .. (tostring(zyfiAC_banlistHistory[nombre].sourceplayername)) .. " , " .. (tostring(zyfiAC_banlistHistory[nombre].reason)) .. " , " .. calcul2 .. _U('days')
					
					TriggerEvent('bansql:sendMessage', source, (nombre .." : ".. resultat))
			else
					for i = 1, #zyfiAC_banlistHistory, 1 do
						if (tostring(zyfiAC_banlistHistory[i].targetplayername)) == tostring(name) then
							local expiration = zyfiAC_banlistHistory[i].expiration
							local timeat     = zyfiAC_banlistHistory[i].timeat
							local calcul1    = expiration - timeat
							local calcul2    = calcul1 / 86400
							local calcul2 	 =  math.ceil(calcul2)					
							local resultat   = (tostring(zyfiAC_banlistHistory[i].targetplayername)) .. " , " .. (tostring(zyfiAC_banlistHistory[i].sourceplayername)) .. " , " .. (tostring(zyfiAC_banlistHistory[i].reason)) .. " , " .. calcul2 .. _U('days')
...
TriggerEvent('es:addGroupCommand', 'unban', Config.permission, function (source, args, user)
  if args[1] ~= nil then
    local name = table.concat(args, " ")
     MySQL.Async.fetchScalar('SELECT identifier FROM zyfiAC_banlist WHERE targetplayername=@name',
    {
        ['@name'] = name
    }, function(identifier)
        if identifier ~= nil then
            MySQL.Async.execute(
            'DELETE FROM zyfiAC_banlist WHERE targetplayername=@name',
            {
              ['@name']  = name
            },
                function ()
...
TriggerEvent('es:addGroupCommand', 'reason', Config.permission, function (source, args, user)
	...
	if name ~= "" then
		if duree ~= nil and duree < 365 then
			...
			MySQL.Async.fetchAll('SELECT * FROM zyfiAC_baninfo WHERE playername = @playername', 
			{
				['@playername'] = name
			}, function(data)
...
AddEventHandler('bansql:sendMessage', function(source, message)
	if source ~= 0 then
		TriggerClientEvent('chat:addMessage', source, { args = { '^1zyfiAC_banlist', message } } )
	else
...
function ban(source,identifier,license,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)
	...
	if expiration < os.time() then
		...
		MySQL.Async.execute(
                'INSERT INTO zyfiAC_banlist (identifier,license,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent) VALUES (@identifier,@license,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent)',
                { 
				['@identifier']       = identifier,
				['@license']          = license,
				['@liveid']           = liveid,
		...
		MySQL.Async.execute(
                'INSERT INTO zyfiAC_banlisthistory (identifier,license,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent) VALUES (@identifier,@license,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent)',
                { 
				['@identifier']       = identifier,
				['@license']          = license,
				['@liveid']           = liveid,
...
function loadzyfiAC_banlist()
  MySQL.Async.fetchAll(
    'SELECT * FROM zyfiAC_banlist',
    {},
    function (data)
      zyfiAC_banlist = {}
...
function loadzyfiAC_banlistHistory()
  MySQL.Async.fetchAll(
    'SELECT * FROM zyfiAC_banlisthistory',
    {},
    function (data)
      zyfiAC_banlistHistory = {}

      for i=1, #data, 1 do
        table.insert(zyfiAC_banlistHistory, {
			identifier       = data[i].identifier,
			license          = data[i].license,
			liveid           = data[i].liveid,
			xblid            = data[i].xblid,
...
MySQL.Async.execute(
            'DELETE FROM zyfiAC_banlist WHERE identifier=@identifier',
            {
              ['@identifier']  = identifier
            },
                function ()
...
AddEventHandler('playerConnecting', function (playerName,setKickReason)
	...
	--Si zyfiAC_banlist pas chargée
	if (zyfiAC_banlist == {}) then
		Citizen.Wait(1000)
	end
	...
	for i = 1, #zyfiAC_banlist, 1 do
		if 
			((tostring(zyfiAC_banlist[i].identifier)) == tostring(steamID) 
			or (tostring(zyfiAC_banlist[i].license)) == tostring(license) 
			or (tostring(zyfiAC_banlist[i].liveid)) == tostring(liveid) 
			or (tostring(zyfiAC_banlist[i].xblid)) == tostring(xblid) 
			or (tostring(zyfiAC_banlist[i].discord)) == tostring(discord) 
			or (tostring(zyfiAC_banlist[i].playerip)) == tostring(playerip)) 
		then

			if (tonumber(zyfiAC_banlist[i].permanent)) == 1 then

				setKickReason(_U('you_have_been_permabanned') .. zyfiAC_banlist[i].reason)
				CancelEvent()
				break

			elseif (tonumber(zyfiAC_banlist[i].expiration)) > os.time() then

				local tempsrestant     = (((tonumber(zyfiAC_banlist[i].expiration)) - os.time())/60)
				if tempsrestant >= 1440 then
					...
					local txtminutes = math.ceil(minutes)
						setKickReason(_U('you_have_been_banned') .. zyfiAC_banlist[i].reason .. _U('time_left') .. txtday .. _U('days') ..txthrs .. _U('hours') ..txtminutes .. _U('minutes'))
						CancelEvent()
						break
			...
			elseif (tonumber(zyfiAC_banlist[i].expiration)) < os.time() and (tonumber(zyfiAC_banlist[i].permanent)) == 0 then
...
AddEventHandler('es:playerLoaded',function(source)
  ...
  Wait(5000)
	...
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		...
		MySQL.Async.fetchAll('SELECT * FROM `zyfiAC_baninfo` WHERE `identifier` = @identifier', {
			['@identifier'] = steamID
		}, function(data)
		local found = false
			for i=1, #data, 1 do
			...
			if not found then
				MySQL.Async.execute('INSERT INTO zyfiAC_baninfo (identifier,license,liveid,xblid,discord,playerip,playername) VALUES (@identifier,@license,@liveid,@xblid,@discord,@playerip,@playername)', 
					{ 
					['@identifier'] = steamID,
					['@license']    = license,
					['@liveid']     = liveid,
				...
				MySQL.Async.execute('UPDATE `zyfiAC_baninfo` SET `license` = @license, `liveid` = @liveid, `xblid` = @xblid, `discord` = @discord, `playerip` = @playerip, `playername` = @playername WHERE `identifier` = @identifier', 
					{ 
					['@identifier'] = steamID,
					['@license']    = license,
					['@liveid']     = liveid,
...
CreateThread(function()
	while true do
		Wait(1000)
		if zyfiAC_banlistLoad == false then
			loadzyfiAC_banlist()
			if zyfiAC_banlist ~= {} then
				print(_U('zyfiAC_banlist_loaded'))
				zyfiAC_banlistLoad = true
			else
				print(_U('zyfiAC_banlist_starterror'))
			end
		end
		if zyfiAC_banlistHistoryLoad == false then
			loadzyfiAC_banlistHistory()
			if zyfiAC_banlistHistory ~= {} then
				print(_U('banhistory_loaded'))
				zyfiAC_banlistHistoryLoad = true
			else
				print(_U('zyfiAC_banlist_starterror'))
			end
		end
